...
class DhanBroker(BaseBroker):
    ...
    def get_quotes(self, symbols: List[str]) -> Dict[str, float]:
        """Get LTP quotes for symbols using Dhan quote_data."""
        if not symbols:
            return {}
        if not self.api:
            if not self.connect():
    ...
    @staticmethod
    def _extract_ltp(item: dict) -> Optional[float]:
        for key in ("LTP", "ltp", "lastTradedPrice", "lastPrice", "last_trade_price"):
            if key in item and item[key] is not None:
                try:
